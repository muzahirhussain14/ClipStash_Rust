...
impl<'a> Renderer<'a> {
    ...
    fn convert_to_values<S> (serialize: &S) -> serde_json::Value
        where 
            S: serde::Serialize + std::fmt::Debug,
    {
        serde_json::to_value(&serialize).expect("failed to convert structure to value")
    }
    ...
    {
        let mut value = Self::convert_to_values(&context);
